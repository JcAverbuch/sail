...
function parseRealtime2(txt: string) {
  ...
  let dataIdx = 1;
  if (/[A-Za-z]/.test(rows[1])) dataIdx = 2;          // skip units line if present
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!id) return json({ error: "id required (e.g. /api/buoy?id=46232)" }, 400);
  ...
  try {
    ...
    return json({ id, ok: true, obs }, 200, { "cache-control": "s-maxage=300, stale-while-revalidate=900" });
  } catch (e: any) {
