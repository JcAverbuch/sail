...
// Convert "15 mph" / "10 to 20 mph" to a single mph number
function mphFromString(s?: string | null): number | null {
  if (!s) return null;
  // pick the max if it's a range like "10 to 20 mph"
...
// Convert cardinal like "NW" or "E" to approx degrees (meteorological “from”)
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const hours = Number(url.searchParams.get("hours") ?? 12); // default 12 hours
  ...
  try {
    ...
    // Normalize to a compact structure your UI can use
    const out = periods.slice(0, Math.max(1, Math.min(hours, 48))).map((p: any) => {
      ...
      const kt = mph != null ? mph * MPH_TO_KT : null;
      const gustKt = gustMph != null ? gustMph * MPH_TO_KT : null;
      const dirTxt = (p?.windDirection as string | undefined) || null;
      const dirDeg = dirTextToDeg(dirTxt);
      return {
        time: p?.startTime || null,
        windKts: kt != null ? +kt.toFixed(1) : null,
        gustKts: gustKt != null ? +gustKt.toFixed(1) : null,
        dirText: dirTxt,
